# diagnostic_300 Manifest Validation Report

- Overall: **PASS**
- Seed: `20260619`
- Catalog SHA-256: `0ebbad4d9a0fa35e3e37f38d964b1d04fa77207870a66048221ec1461044b24e`

## Checks

- [x] `exactly_300_entries`
- [x] `300_unique_ids`
- [x] `split_sizes_exact`
- [x] `no_cross_split_duplicates`
- [x] `all_entries_in_catalog`
- [x] `all_paths_exist`
- [x] `mechanic_coverage_at_least_random_baseline`
- [x] `seed_recorded`
- [x] `catalog_checksum_recorded`

## Mechanic Enrichment

- Selected mechanic-flag total: 2090
- Random-baseline mechanic-flag total: 1633

| Mechanic | Selected | Random baseline |
| --- | ---: | ---: |
| `tera` | 280 | 257 |
| `boosts_drops` | 300 | 299 |
| `major_status` | 262 | 224 |
| `item_reveal_loss` | 259 | 218 |
| `ability_reveal_change_suppression` | 249 | 242 |
| `type_change` | 48 | 21 |
| `transform` | 29 | 12 |
| `illusion` | 19 | 6 |
| `weather` | 134 | 75 |
| `terrain` | 60 | 28 |
| `screens` | 39 | 16 |
| `tailwind` | 2 | 0 |
| `hazards` | 188 | 147 |
| `recharge_lock_constraints` | 41 | 18 |
| `encore` | 102 | 43 |
| `disable` | 36 | 4 |
| `taunt` | 42 | 23 |
| `choice_like_constraints` | 0 | 0 |
